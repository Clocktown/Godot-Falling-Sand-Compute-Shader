...
Because it is so fast, it is very much feasible to run more than two substeps every physics step to speed up the simulation. My GPU can still handle setting the playback to 16x in the editor on a 4000x4000 simulation resolution and requires under one millisecond for the compute.
